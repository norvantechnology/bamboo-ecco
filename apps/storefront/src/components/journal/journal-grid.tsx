...
export function JournalGrid({ posts }: { posts: BlogPost[] }) {
  return (
    <MotionStaggerContainer
      staggerDelay={0.08}
      className="mt-6 grid gap-4 sm:mt-10 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4"
    >
      {posts.map((post) => (
        <MotionStaggerChild key={post._id} variants={childFadeUpVariants}>
          ...
          >
            <div>
              {post.heroImage ? (
                <div className="relative aspect-[16/9] w-full overflow-hidden rounded-xl bg-muted/20 mb-4">
                  <Image
                    src={post.heroImage}
                    alt={post.title}
                    fill
